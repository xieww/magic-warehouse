- [Promise 实现](#promise-实现)
  - [简版 Promise 实现](#简版-promise-实现)
  - [符合 Promise/A+规范的 Promise 实现](#符合-promisea规范的-promise-实现)
...
- 首先两个函数都得判断当前状态是否为等待中，因为规范中规定只有等待态才可以改变状态
- 将当前状态更改为对应状态，并且将传入的值赋值给 `that.value`
- 遍历回调数组并执行
